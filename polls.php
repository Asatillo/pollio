...
<body >
    <nav class="navbar navbar-expand-lg bg-body-tertiary">
        <div class="container-fluid">
            ...
            <ul class="navbar-nav">
                <li class="nav-item">
                <a class="nav-link" aria-current="page" href="#">Polls</a>
                </li>
                <li class="nav-item">
    ...
    <div class="container">
        <h2 class="text-center">Freshhhh polls</h2><hr>
        <?php
        ...
        foreach($polls as $poll){
            ...
            if (!$expired){
                echo '<form method="POST" action="poll.php" class="mb-3">';
                ...
                if(array_key_exists($_SESSION['user'], $poll['voted'])){
                    echo '<div class="form-group">
                            <div><b>My vote:</b> ' . implode(', ', $poll['voted'][$_SESSION['user']]) . '</div>
                        </div>';

                    echo '<div class="d-flex justify-content">';

                    echo '<form method="POST" action="poll.php">
                        <input type="hidden" name="id" value="'. $poll['id'] .'">
                        <input name="change" type="submit" value="Change my vote" class="btn btn-primary mt-1 mx-2">
                    </form>';

                    echo '<form method="POST" action="polls.php">
                        <input type="hidden" name="id" value="'. $poll['id'] .'">
                        <input name="retract" type="submit" value="Retract my vote" class="btn btn-danger mt-1 mx-2">
                    </form>';

                    echo '</div>';
                }else{
                    echo '<div><b>Votes:</b> ' . array_sum($poll['answers']) . '</div>';
                    echo '<button type="submit" class="btn btn-primary mt-1">Vote</button>';
                    echo '</form>';
                }
                echo '<hr>';
            }
        }
        echo '<h2 class="text-center">Expired polls</h2><hr>';
        foreach($polls as $poll){
            $expired = time() > strtotime($poll['deadline']);
            if (!array_key_exists($_SESSION['user'], $poll['voted']) && $expired){
                echo '<form method="POST" action="poll.php" class="mb-3">';
                echo '<input type="hidden" name="id" value="'. $poll['id'] .'">';
                echo '<div class="form-group">';
                echo '<div><b>' . $poll['question'] . '</b></div>';
                ...
                echo '<div><b>Votes:</b> ' . array_sum($poll['answers']) . '</div>';
                echo '<ul>';
                foreach($poll['answers'] as $answer => $count){
                    echo "<li>" . $answer . ": " . $count . "</li>";
                }
                echo '</ul>';
                echo '</form>';
                echo '<hr>';
